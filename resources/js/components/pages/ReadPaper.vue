<template>
    <div class="cont">
        <!-- ボタンエリア -->
        <div v-if="$store.state.login" class="m_btns">
            <!-- ブックマークボタン（切り替え） -->
            <v-icon
                v-if="!bookmarkToggle"
                size="60px"
            ...
            <img
                class="icon"
                src="/img/sns/Twitter_Logo_Blue.png"
                alt
                @click="openTweet"
            />
        </div>
            ...
            axios
                ...
                .then(res => {
                    ...
                    this.showText.forEach((el, i) => {
                        ...
                        if (i % 2 == 1) {
                            $("#insert_area").append(
                                "<div class='story_text' style='width:350px;height:650px; writing-mode:vertical-rl; text-orientation: upright;background-color:#fff;font-size:20px;'>" +
                                    "<span class='text_area'><br/>" +
                                    el.text +
                                    "</span></div>"
                            );
